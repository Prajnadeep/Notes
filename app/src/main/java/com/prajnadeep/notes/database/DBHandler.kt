...
class DBHandler(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, 1) {
    ...
    fun getUserNotes(AccountID: String) : MutableList<User>{
        ...
        if(result.moveToFirst()){
            do {
                ...
                user.noteTitle = result.getString(result.getColumnIndex(COL_USER_NOTE_TITLE)).toString()
                user.noteDesc = result.getString(result.getColumnIndex(COL_USER_NOTE_DESC)).toString()
                if (result.getString(result.getColumnIndex(COL_USER_NOTE_TITLE)).toString() == "temp") {
    ...
    fun deleteOneNote(NoteTitle: String){
        val db = this.writableDatabase
        val q = db.delete(TABLE_NAME_USERS,"User_noteTitle=?", arrayOf(NoteTitle))
        db.close()
    }

    fun updateData(UserID: Int,NoteTitle: String,NoteDesc:String) {
        val db = this.writableDatabase
        val cv = ContentValues()
        cv.put(COL_USER_NOTE_TITLE,NoteTitle)
        cv.put(COL_USER_NOTE_DESC,NoteDesc)
        val q = db.update(TABLE_NAME_USERS,cv,"User_ID=?", arrayOf(UserID.toString()))
        db.close()
    }
